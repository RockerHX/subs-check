...
if command -v go >/dev/null 2>&1; then
  cd "$REPO_ROOT"
  exec go run ./cmd/home-filter -config "$CONFIG_PATH" -serve "$LISTEN_ADDR" "$@"
fi

printf '%s\n' "home-filter service requires Go in PATH, or set SUBS_CHECK_HOME_FILTER_BIN to a prebuilt home-filter binary." >&2
exit 127
